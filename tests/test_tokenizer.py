...
def test_tokenize_raven_snippet():
    ...
    assert all(
        isinstance(token, str) for token in result
    ), f"Tokenizer failed to return strings list elements for: f{text}"
...
def test_tokenize_compare_with_bash():
    ...
    bash_result = int(
        subprocess.check_output(f"echo '{text}' | wc -w", shell=True).decode().strip()
    )
    assert (
        python_result == bash_result
    ), "different results from python tokenizer and bash "
