...
def test_count_raven_snippet():
    ...
    assert result['outpour'] == 1, "failure: expected 'outpour' to appear once in f{text}"
...
def test_clean_full_file(filename):
    ...
    assert all(isinstance(k, str) and isinstance(v,int) for k,v in result.items()), "count_words failed on k,v types for The Raven file"
...
def test_clean_all_english_files():
    ...
    assert all(isinstance(k, str) and isinstance(v,int) for k,v in result.items()), "count_words failed on k,v types for all English files"
...
def test_count_words_compare_with_bash():
    ...
    assert python_result == bash_result, "different results from python count_words and bash counting"
