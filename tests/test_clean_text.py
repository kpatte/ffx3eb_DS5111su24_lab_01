...
def test_clean_raven_snippet():
    ...
    assert not re.search(
        r"[^\w\s]", clean_text(text)
    ), f"clean_text failed to remove all punctuation in: {text}"
...
def test_clean_french():
    ...
    assert not re.search(
        r"[^\w\s]", result
    ), f"clean_text failed to remove all punctuation in french text"
...
def test_clean_full_file(filename):
    ...
    I should get no punctuation or upper case
    """
    ...
    assert not re.search(
        r"[^\w\s]", result
    ), f"clean_text returned empty list for file: {filename}"
...
def test_clean_text_compare_with_bash():
    ...
    ), "different results from python clean_text and bash cleaning"
